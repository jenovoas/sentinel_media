...
const CircularProgress: React.FC<CircularProgressProps> = ({ value, max, color, label }) => {
...
const FactoryView: React.FC = () => {
    ...
    const [resonance, setResonance] = useState<number[]>(new Array(12).fill(0));
    ...
    useEffect(() => {
        ...
        const setupListeners = async () => {
            ...
            unlistenFactoryCompleted = await listen('tarea-fabrica-completada', (event: { payload: { success: boolean, message: string } }) => {
            ...
            await listen('ring0-tick', async () => {
                try {
                    const data = await invoke<number[]>('get_crystal_resonance');
                    setResonance(data);
                } catch (e) {
    ...
    const generatingOps = ops.filter((o: OpEntry) => (o.status === 'Running' || o.status === 'Pending') && o.op_type.startsWith('generate'));
    const processingOps = ops.filter((o: OpEntry) => (o.status === 'Running' || o.status === 'Pending') && o.op_type === 'stitch');
    const publishedOps = ops.filter((o: OpEntry) => o.status === 'Completed' || o.status === 'Done');
                ...
                <div className="flex flex-col md:flex-row gap-8">
                    ...
                    <div className="border-l border-white/5 pl-8 space-y-6">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {[
                                { id: 'gemini', label: 'Gemini 2.0', sub: 'Flash / Flash-lite', icon: <Sparkles size={14} />, color: 'text-sentinel-blue', border: 'border-sentinel-blue/20' },
                                ...
                                { id: 'antigravity', label: 'Antigravity', sub: 'Neural Engine', icon: <Database size={14} />, color: 'text-purple-400', border: 'border-purple-400/20' },
                                { id: 'perplexity', label: 'Perplexity', sub: 'Researched', icon: <Globe size={14} />, color: 'text-cyan-400', border: 'border-cyan-400/20' },
                            ].map((prov) => (
                                <button
                                    key={prov.id}
                ...
                <div className="lg:col-span-3 space-y-6">
                    <div className="p-8 rounded-[2.5rem] bg-white/[0.01] border border-white/5 space-y-6">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-xs font-black text-white/40 uppercase tracking-widest">Vertex AI (Veo 3.0)</h3>
                        </div>
                ...
                <div className="flex items-center gap-6">
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/[0.02] border border-white/5">
                        ...
                        <span className="text-[9px] font-black text-white/40 uppercase tracking-widest font-mono">
                            SYSTEM STATUS: <span className='text-sentinel-green'>ONLINE</span>
                        </span>
                    </div>
                    
                    {/* Visualizador de Resonancia del Ring 0 */}
                    <div className="flex items-end gap-[2px] h-6 px-3 border-l border-white/5">
                        {resonance.map((val: number, i: number) => (
                            <motion.div
                                key={i}
                                animate={{ height: `${Math.min(100, Math.max(10, val * 100))}%` }}
                                className="w-[3px] bg-sentinel-blue/40 rounded-full"
                                transition={{ type: "spring", stiffness: 300, damping: 20 }}
                            />
                        ))}
                        <span className="ml-2 text-[8px] font-mono text-white/20 uppercase">Ring 0 Lattice</span>
                    </div>
